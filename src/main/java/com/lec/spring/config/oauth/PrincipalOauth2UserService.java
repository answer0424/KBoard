...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if (user == null) { // 미 가입자인 경우에만 회원가입 진행
            ...
            Authority auth = authorityRepository.findByName("ROLE_MEMBER"); // ROLE_MEMBER 주입을 위함 (Service를 주입받으면 편하지만 순환참조의 문제때문에 Repository를 가져와서 구현해야 함)
        ...
        principalDetails.setAuthorityRepository(authorityRepository);   // 꼭 !!!!!

        return principalDetails;    // 결국 OAuth2User를 리턴한 셈!!!!!!!!!!
